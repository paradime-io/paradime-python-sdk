...
class ArtifactParser:
    ...
    def extract_run_results_data(self, parsed_artifacts: ParsedArtifacts) -> List[Dict[str, Any]]:
        ...
        if hasattr(run_results, "results"):
            ...
            run_results.metadata if hasattr(run_results, "metadata") else {}
        else:
            # Raw dictionary
            ...
            extracted_data.append(
                {
                    ...
                    "thread_id": (
                        ...
                        else result.get("thread_id")
                    ),
                    "adapter_response": (
                        getattr(result, "adapter_response", None)
                        if hasattr(result, "adapter_response")
                        else result.get("adapter_response")
                    ),
                }
            )
    ...
    def extract_source_freshness_data(
        self, parsed_artifacts: ParsedArtifacts
    ) -> List[Dict[str, Any]]:
        ...
        if manifest:
            manifest_sources = (
                manifest.sources if hasattr(manifest, "sources") else manifest.get("sources", {})
            )
            for source_id, source_node in manifest_sources.items():
                if hasattr(source_node, "unique_id"):
                    ...
                    metadata = {
                        ...
                        "owner": (
                            getattr(source_node, "meta", {}).get("owner")
                            if isinstance(getattr(source_node, "meta", {}), dict)
                            else None
                        ),
                    ...
                    metadata = {
                        ...
                        "owner": (
                            source_node.get("meta", {}).get("owner")
                            if isinstance(source_node.get("meta", {}), dict)
                            else None
                        ),
        ...
        for result in results_list:
            ...
            if criteria:
                if hasattr(criteria, "error_after"):
                    ...
                    error_after = criteria["error_after"]
                    error_after_hours = (
                        error_after.get("count") if isinstance(error_after, dict) else None
                    )
                ...
                if hasattr(criteria, "warn_after"):
                    ...
                    warn_after = criteria["warn_after"]
                    warn_after_hours = (
                        warn_after.get("count") if isinstance(warn_after, dict) else None
                    )
    ...
    def extract_seed_data(self, parsed_artifacts: ParsedArtifacts) -> List[Dict[str, Any]]:
        ...
        if manifest:
            manifest_nodes = (
                manifest.nodes if hasattr(manifest, "nodes") else manifest.get("nodes", {})
            )
            for unique_id, node in manifest_nodes.items():
                ...
                if hasattr(node, "name"):
                    ...
                    compiled_code = getattr(node, "compiled_code", None) or getattr(
                        node, "compiled_sql", None
                    )
                ...
                extracted_data.append(
                    {
                        ...
                        "run_elapsed_time": run_data.get(
                            "execution_time"
                        ),  # Same as execution_time for seeds
                        # Timing information
                        ...
                        "run_generated_at": run_data.get(
                            "execute_completed_at"
                        ),  # Use execute completion as run generation
                        # Code and SQL
        ...
        if manifest:
            ...
            for unique_id, node in manifest_nodes.items():
                ...
                if hasattr(node, "name"):
                    ...
                    columns = dict(getattr(node, "columns", {}))
                    depends_on = (
                        list(getattr(node, "depends_on", {}).get("nodes", []))
                        if hasattr(getattr(node, "depends_on", {}), "get")
                        else []
                    )
                else:
                    # Raw dictionary
                ...
                extracted_data.append(
                    {
                        ...
                        "run_elapsed_time": run_data.get(
                            "execution_time"
                        ),  # Same as execution_time for snapshots
                        # Timing information
                        "compile_started_at": run_data.get("compile_started_at"),
                        "compile_completed_at": run_data.get("compile_completed_at"),
                        "execute_started_at": run_data.get("execute_started_at"),
        ...
        for unique_id, node in nodes.items():
            if hasattr(node, "resource_type"):
                ...
                materialized = (
                    ...
                    else getattr(config, "materialized", None)
                )
                ...
                depends_on_data = node.get("depends_on", {})
                depends_on = (
                    depends_on_data.get("nodes", []) if isinstance(depends_on_data, dict) else []
                )
            ...
            extracted_data.append(
                {
                    ...
                    "original_file_path": (
                        getattr(node, "original_file_path", None)
                        if hasattr(node, "original_file_path")
                        else node.get("original_file_path")
                    ),
                    "root_path": (
                        getattr(node, "root_path", None)
                        if hasattr(node, "root_path")
                        else node.get("root_path")
                    ),
                }
            )
    ...
    def _compute_dependency_graph(self, model_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Compute children relationships and separate parents by type"""
        {item["unique_id"]: item for item in model_data}
    ...
    def enrich_run_results_with_manifest(
        self, run_results_data: List[Dict[str, Any]], model_metadata: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
                ...
                result.update(
                    {
                        ...
                        "model_type": (
                            metadata.get("config", {}).get("materialized", "unknown")
