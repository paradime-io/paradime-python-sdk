...
@click.option(
    "--sync-id",
    multiple=True,
    help="The ID(s) of the Hightouch sync(s) you want to trigger",
    required=True,
)
@click.option(
    "--full-resync",
...
def hightouch_sync(
    ...
    """
    Trigger sync for Hightouch syncs.

    Requires the HIGHTOUCH_API_TOKEN environment variable to be set.
    """
    click.echo(f"Starting sync for {len(sync_id)} Hightouch sync(s)...")
...
@click.option(
    "--sync-sequence-id",
    multiple=True,
    help="The ID(s) of the Hightouch sync sequence(s) you want to trigger",
    required=True,
)
@click.option(
    "--wait-for-completion",
    is_flag=True,
    help="Wait for sync sequences to complete before returning",
    default=True,
)
@click.option(
    "--timeout-minutes",
