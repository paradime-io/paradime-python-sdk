...
def list_dataflow_jobs(
    *,
    ...
    """List active and recent Dataflow jobs."""
    from google.cloud import dataflow_v1beta3

    jobs_client = _get_jobs_client(service_account_key_file)

    print(f"\n🔍 Listing Dataflow jobs in project '{project}', location '{location}'")

    jobs = list(
        jobs_client.list_jobs(
    ...
    for i, job in enumerate(jobs, 1):
        state_name = dataflow_v1beta3.JobState(job.current_state).name
        state_emoji = (
            "✅"
            if state_name == "JOB_STATE_DONE"
                ...
                if state_name == "JOB_STATE_RUNNING"
                    ...
                    if state_name == "JOB_STATE_FAILED"
                        ...
                        "⏳"
                        if state_name in ("JOB_STATE_PENDING", "JOB_STATE_QUEUED")
                        else "⚠️" if state_name == "JOB_STATE_CANCELLED" else "❓"
                    )
                )
            )
        )
        ...
        print(f"   Type: {dataflow_v1beta3.JobType(job.type_).name}")
        if job.create_time:
            print(f"   Created: {job.create_time}")
        console_url = (
            f"https://console.cloud.google.com/dataflow/jobs/{location}/{job.id}"
            f"?project={project}"
        )
